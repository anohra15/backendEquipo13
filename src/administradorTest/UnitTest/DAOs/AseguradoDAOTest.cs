...
namespace administradorTest.UnitTest.DAOs
{
    public class AseguradoDAOTest
    {
        ...
        [Fact(DisplayName = "Valida si me inserta un asegurado")] //acá debería traer una excepcion
        public Task CreateInsureFalse()
        {
            AseguradoDTO dto = new AseguradoDTO();
            var result = Assert.Throws<RCVExceptions>(()=>this._dao.createInsured(dto)); 
            Assert.Equal(result.Mensaje, "No se puede crear, detalles: ");
            return Task.CompletedTask;
        }
    }
}
