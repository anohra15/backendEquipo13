...
namespace RCVUcabBackend.Persistence.DAOs.Implementations
{
    public class ProveedorDao:IProveedorDAO
    {
        ...
        public ProveedorDao(IRCVDbContext context)
        {
            _context = context;
        }
        ...
        public bool AsignarMarcaExistente(IRCVDbContext context,MarcaDTO marcaValidar)
        {
            if (verificarMarca( context, marcaValidar))
            {
                var marca = context.Marcas.
                    Where(b => b.nombre.Equals(marcaValidar.nombre)).
                    First();
        ...
        public int CreateProveedor(ProveedorDTO proveedor)
        {
            ...
            try
            {
                ...
                {
                    error++;
                    mensajeError = "No se puede crar un proveedor si alguno de estos datos esta vacio:nombre del proveedor, direccion, telefono y marcas ";
                    throw new RCVExceptions(mensajeError);
                }
                else
                {
